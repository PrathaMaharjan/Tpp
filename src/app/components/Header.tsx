...
export default function Header() {
  ...
  return (
    ...
    >
      {showBanner && (
        <div className="bg-[#4fa1b0] text-white text-[11px] sm:text-xs md:text-sm px-4 sm:px-6 py-2 relative flex items-center justify-center font-medium">
        ...
        <div className="flex lg:hidden items-center gap-2 shrink-0 z-10">
          ...
          <button
            ...
            className={`flex items-center justify-center w-10 h-10 rounded-xl border transition-all duration-300 ${isMenuOpen
              ? 'border-[#2596be]/30 bg-[#2596be]/10 text-[#2596be]'
              : isTransparent
                ? 'border-white/40 bg-white/15 text-white backdrop-blur-sm hover:border-white hover:bg-white/25'
                : 'border-slate-200 bg-white/80 text-slate-800 hover:border-[#2596be] hover:text-[#2596be]'
              }`}
      ...
      >
        <nav className="flex flex-col px-4 sm:px-6 py-4 gap-1">
            ...
            return (
              <Link
                ...
                className={`px-4 py-3 rounded-xl text-sm font-semibold transition-colors duration-200 ${isActive
                  ? 'bg-gradient-to-r from-[#2596be] via-[#4fa1b0] to-[#67bed9] text-white'
                  : 'text-slate-700 hover:bg-[#2596be]/10 hover:text-[#2596be]'
                  }`}
              >
                {link.name}
              </Link>
          ...
          <Link
            href="/booking"
            className="mt-3 flex items-center justify-center gap-2 px-5 py-3 bg-gradient-to-r from-[#2596be] via-[#4fa1b0] to-[#67bed9] text-white text-sm font-semibold rounded-xl active:scale-[0.99] transition-transform duration-200"
          >
            <Calendar size={16} />
            <span>Book Appointment</span>
          </Link>
